...
public class Game1Fragment extends Fragment {
    ...
    public View onCreateView(final LayoutInflater inflater, final ViewGroup container,
        ...
        fragmentView = inflater.inflate(R.layout.fragment_game1, container, false);

        progressBar = (ProgressBar) fragmentView.findViewById(R.id.progress_game1);

        textVotes = (TextView) fragmentView.findViewById(R.id.text_votes1);
        textLife = (TextView) fragmentView.findViewById(R.id.text_life1);

        voteButton = (AppCompatButton) fragmentView.findViewById(R.id.button_vote_game1);
        messageButton = (ImageButton) fragmentView.findViewById(R.id.btn_goto_message1);
                            ...
                            public void onDataChange(DataSnapshot dataSnapshot) {
                                ...
                                for (DataSnapshot child : dataSnapshot.getChildren()) {
                                    ...
                                    if (firebaseAuth.getCurrentUser() != null) {
                                        if ((thread.userId1.equals(game.playerIds.get(0))
                                                && thread.userId2.equals(firebaseAuth.getCurrentUser().getUid()))
                                                || (thread.userId2.equals(game.playerIds.get(0))
                                ...
                                if (!hasBeenInitialized) {
                                    DMThread dmThread = new DMThread(game.players.get(0), currentUser,
                                            game.playerIds.get(0), firebaseAuth.getCurrentUser().getUid(), new ArrayList<Message>());
        ...
        floatingActionButton = (FloatingActionButton) fragmentView.findViewById(R.id.button_goto_game2);
        floatingActionButton.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                ((ViewPager) container.findViewById(R.id.view_pager_game)).setCurrentItem(1, true);
            }
        });
        ...
        database.child("games").child(getActivity().getIntent().getStringExtra("gameId")).addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(DataSnapshot dataSnapshot) {
                ...
                if (!firebaseAuth.getCurrentUser().getUid().equals(game.playerIds.get(0)) && !game.playerIds.get(0).equals("JWsOb4iV7rYti9cSSqJgPjygxEK2"))
                    messageButton.setVisibility(View.VISIBLE);

                mediaReference = storage.child("Images").child(game.mediaUrls.get(0));

                imageView = (ImageView) fragmentView.findViewById(R.id.image_game1);
                                        ...
                                        public void onResourceReady(GlideDrawable resource, GlideAnimation<? super GlideDrawable> animation) {
                                            ...
                                            voteButton.setOnClickListener(new View.OnClickListener() {
                                                ...
                                                public void onClick(View view) {
                                                    ...
                                                    voteButton.setSupportBackgroundTintList(ColorStateList.valueOf(getResources().getColor(R.color.green)));
                                                    voteButton.setText("Voted");
                                                    Button voteButton2 = (Button) container.findViewById(R.id.button_vote_game2);
                                                    voteButton2.setVisibility(View.GONE);
                                                    voters.add(firebaseAuth.getCurrentUser().getUid());
                                                    gameValues.put("voters1", voters);
                                                    database.child("games").child(getActivity().getIntent().getStringExtra("gameId")).updateChildren(gameValues, new DatabaseReference.CompletionListener() {
                                                        @Override
                                                        public void onComplete(DatabaseError databaseError, DatabaseReference databaseReference) {
                                                            Notification notification = new Notification(game.playerIds.get(0), "Someone has voted for your photo");
                                                            database.child("notifications").push().setValue(notification);
                                                            MyApplication.getInstance().sendFCM(game.players.get(0).fcmId, "Someone has voted for your photo");
                                                        }
                                                    });
                                                }
                                            });
